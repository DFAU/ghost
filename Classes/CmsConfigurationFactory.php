...
class CmsConfigurationFactory
{
    ...
    protected static function getConnectionConfiguration(
        ...
        if (!isset($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['ghost']['connections'][$connectionName])) {
            throw new UndefinedConnectionException('The connection name "' . $connectionName . '" is not defined in $GLOBALS[\'TYPO3_CONF_VARS\'][\'EXTCONF\'][\'ghost\'][\'connections\']',
                1494840181);
        }
        ...
        if ($connectionName !== self::DEFAULT_CONNECTION_NAME) {
            ArrayUtility::mergeRecursiveWithOverrule($connectionConfiguration,
        ...
        if ($configurationPath !== null) {
            try {
                ...
                throw new IncompleteConnectionConfigurationException('The connection configuration for connection name "' . $connectionName . '" is missing the configuration path "' . $configurationPath . '"',
                    1494844217);
            }
        }
     ...
     * @throws UndefinedConnectionException
     */
    public static function getQueueFactoryForConnectionName(string $connectionName = self::DEFAULT_CONNECTION_NAME
    ): QueueFactory {
        ...
        if (!isset($queueFactories[$connectionName])) {
            ...
            $queueFactories[$connectionName] = call_user_func_array([GeneralUtility::class, 'makeInstance'],
